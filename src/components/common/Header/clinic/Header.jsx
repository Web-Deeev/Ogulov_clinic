// src/components/common/Header/clinic/Header.jsx
import React from 'react';
import { NavLink, Link, useLocation } from 'react-router-dom'; // Импортируем чистый Link
import { HashLink } from 'react-router-hash-link'; 
import './Header.css';

// Меню клиники на честные многостраничные роуты из App.jsx
const clinicMenu = [
...
export default function Header() {
  ...
  // 🎯 СЕНЬОР-ФИКС ДЛЯ HASHROUTER: Проверяем и pathname, и реальный хэш в адресной строке
    ...
    <header className="site-header">
      ...
      <div className="site-main-header">
        <div className="container site-main-header__inner">
          ...
          {/* 🎯 СЕНЬОР-ФИКС: Прописываем пути в 'to' строго с учетом хэша через '#' 
              ...
              <li key={item.title}>
                {item.href.includes('#') ? (
                  ...
                  <NavLink
                    to={item.href}
                    /* 
                      🎯 СЕНЬОР-ФИКС: Нативный флаг строгого соответствия пути.
                      Убирает баг "вечно горящей" кнопки Главная при выборе других страниц клиники.
                    */
                    end
                    className={({ isActive }) =>
